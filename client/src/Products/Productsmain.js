...
function Productsmain() {
  ...
  const bestlist = bests.map((v) => (
    <Productsmainbest name={v[0]} price={v[1]} />
  ));
  const news = [
    ["상품이름", 4512],
    ["상품이름", 4545452],
    ...
    <div className="Productsmain">
      <div className="advertisement">
        ...
        <div className="main_best">
          <div className="main_bestscroll">{bestlist}</div>
        </div>
        <div className="prototype"></div>
        <h1>New</h1>
        <div className="main_new">
          <div className="main_newscroll">{newlist}</div>
        </div>
      </div>
    </div>
  );
